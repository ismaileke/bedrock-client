...
pub struct ResourcePacksInfo {
    ...
    pub behaviour_packs: Vec<BehaviourPack>,
...
pub fn decode(bytes: Vec<u8>) -> ResourcePacksInfo {
    ...
    let force_server_packs = stream.get_bool();

    let behaviour_pack_count = stream.get_l_short();
    let mut behaviour_packs = Vec::new();
    for _ in 0..behaviour_pack_count {
        ...
        behaviour_packs.push(BehaviourPack{ uuid, version, size_bytes, encryption_key, sub_pack_name, content_id, has_scripts, is_addon_pack });
    }
    ...
    ResourcePacksInfo { must_accept, has_addons, has_scripts, force_server_packs, behaviour_packs, resource_packs, cdn_urls }
}