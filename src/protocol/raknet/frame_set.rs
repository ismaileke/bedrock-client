    ...
    pub fn split_packet(body: Vec<u8>, frame_number_cache: &mut FrameNumberCache) -> Vec<Datagram> {
        ...
        if body.len() > 1300 {
            ...
            for i in 0..=multiple {
                ...
                let frame = Datagram::create_frame(range, RELIABLE_ORDERED, frame_number_cache, Some(Fragment{
    ...
    pub fn from_binary(frame_packet: Vec<u8>) -> Datagram {// fragment handler kısmı yok - belki burda sunucunun sequenceini kontrol ederiz ona göre nack gönderirirz
        ...
        while !stream.feof() {
            ...
            if is_sequenced_or_ordered(reliability) {
                order = Option::from(Order { ordered_frame_index: stream.get_u24_le(), order_channel: stream.get_byte() })
            }

            if has_split {
                fragment = Option::from(Fragment { compound_size: stream.get_u32_be(), compound_id: stream.get_u16_be(), index: stream.get_u32_be() })
            }
