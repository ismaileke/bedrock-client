...
// paketleri sıralı işle ORDERa göre bak paketin düzgünce gelmesini bekle
...
pub struct Client {
    ...
    //last_received_packets: HashMap<i32, Vec<u8>>, // reliable_frame_index: body
    ...
    //last_received_reliable_frame_index: i32,
...
pub async fn create(target_address: &'static str, target_port: u16, client_version: &'static str, debug: bool) -> Option<Client> {
    ...
    Option::from(Client{
        ...
        //last_received_reliable_frame_index: -1,
                    ...
                    if frame_set::is_datagram(packet_id) {
                        ...
                        if (self.last_received_sequence_number + 1) != seq {
                            ...
                            for seq_num in (self.last_received_sequence_number+1)..seq {
                                if !self.last_received_packets.contains_key(&seq_num) {
                                    /*let nack = acknowledge::create(PacketType::NACK, 1, true, Option::from(seq_num), None, None);
                                    self.socket.send(&nack.encode()).expect("ACK Send Error");*/
                                    completed = false;
                                }
                            }
                            if completed {
                                if let Some(seq_number) = self.last_received_packets.keys().max() {
                                    self.last_received_sequence_number = *seq_number;
                                } else {
                        ...
                        /*for frame in &datagram.frames {
                            if let Some(reliable_frame_index) = frame.reliable_frame_index {
                                self.last_received_packets.insert(reliable_frame_index, frame.body.clone());


                                //buraya bak
                                if (self.last_received_reliable_frame_index + 1) != reliable_frame_index {


                                    let mut completed = true;
                                    for seq_num in (self.last_received_reliable_frame_index+1)..reliable_frame_index {
                                        if !self.last_received_packets.contains_key(&seq_num) {
                                            /*let nack = acknowledge::create(PacketType::NACK, 1, true, Option::from(seq_num), None, None);
                                            self.socket.send(&nack.encode()).expect("ACK Send Error");*/
                                            completed = false;
                                    ...
                                    if completed {
                                        if let Some(seq_number) = self.last_received_packets.keys().max() {
                                            self.last_received_reliable_frame_index = *seq_number;
                                        } else {
                                            println!("HashMap NONE.");
                                        }
                                    } else { continue; }
                                    ...
                                    self.last_received_reliable_frame_index = reliable_frame_index;
                                }
                            }
                        ...
                        // bunu orderin başladığı yere koy yani encryption un başladığı yere koy
                        /*for frame in &datagram.frames {
                            if let Some(fragment) = &frame.fragment {
                                let mut map = HashMap::new();
                                map.insert(fragment.index, frame.body.clone());
                                self.last_received_fragment_packets.insert(fragment.compound_id, map);
                                if self.last_received_fragment_packets.get(&fragment.compound_id).unwrap().len() as u32 == fragment.compound_size {
                                    if let Some(order) = &frame.order {
                                        self.last_received_packets.insert(order.ordered_frame_index, frame.body.clone());
                                    }
                        ...
                        let mut sorted_packets: Vec<_> = self.last_received_packets.iter().collect();
                        sorted_packets.sort_by_key(|(&key, _)| key); // bu kodda indexleri sıralıyor da valuelar da onla beraber geliyo (inşallah)

                        for (_sequence_number, datagram) in sorted_packets {
                            for frame in &datagram.frames {
                                ...
                                match packet_type {
                                    ...
                                    PacketType::ConnReqAccepted => {
                                        ...
                                        if self.debug {
                                            println!("--- ConnectionRequestAccepted ---");
                                        ...
                                        let frame_two = Datagram::create_frame(request_network_settings, RELIABLE_ORDERED, &self.frame_number_cache, None);

                                        let datagram = Datagram::create(vec![frame, frame_two], &self.frame_number_cache).to_binary();
                                        self.frame_number_cache.sequence_number += 1;
                                        self.frame_number_cache.reliable_frame_index += 1;
                                        self.frame_number_cache.ordered_frame_index += 1;
                                            ...
                                            if !self.compression_enabled { // Compression Not Enabled AND Encryption Not Enabled
                                                ...
                                                match packet_type {
                                                    BedrockPacketType::NetworkSettings => {
                                                        ...
                                                        let login_data_detail = login::convert_login_chain(&mut self.chain, pkey);
                        ...
                        self.last_received_packets.clear();
                        
                        /*for frame in &datagram.frames {
                            let mut stream = Stream::new(frame.body.clone(), 0);
                            let packet_id = stream.get_byte();
                            let packet_type = PacketType::from_byte(packet_id);
                                    ...
                                    if !self.encryption_enabled {
                                        if !self.compression_enabled { // Compression Not Enabled AND Encryption Not Enabled
                                            ...
                                            match packet_type {
                                                BedrockPacketType::NetworkSettings => {
                                                    let network_settings = network_settings::decode(stream.get_remaining().unwrap());
                                                    println!("--- {}Network Settings{} ---", color_format::COLOR_GOLD, COLOR_WHITE);
                                                    println!("Compression Threshold: {}", if network_settings.compression_threshold == 1 { "COMPRESS_EVERYTHING" } else { "COMPRESS_NOTHING" });
                                                    println!("Compression Algorithm: {}", if network_settings.compression_algorithm == 0 { "ZLIB" } else if network_settings.compression_algorithm == 1 { "SNAPPY" } else { "NONE" });
                                                    println!("Enable Client Throttling: {}", network_settings.enable_client_throttling);
                                                    println!("Client Throttle Threshold: {}", network_settings.client_throttle_threshold);
                                                ...
                                                BedrockPacketType::Disconnect => {
                                                    let disconnect = disconnect::decode(stream.get_remaining().unwrap());
                                                    println!("--- {}Disconnect{} ---", color_format::COLOR_GOLD, COLOR_WHITE);
                                                    println!("Disconnect (string): {}", String::from_utf8(stream.get_remaining().unwrap()).unwrap());
                                                    println!("Reason: {}", disconnect.reason);
                                                    if disconnect.skip_message {
                                                        println!("Message: {}", disconnect.message.unwrap());
                                            ...
                                            let decompressed = game_packet::decompress(&stream.get_remaining().unwrap());
                                            ...
                                            match packet_type {
                                                BedrockPacketType::ServerToClientHandshake => {
                                                    let s_to_c_handshake = server_to_client_handshake::decode(decompressed_stream.get_remaining().unwrap());
                                                    let jwt = String::from_utf8(s_to_c_handshake.jwt).unwrap();
                                                    println!("--- {}Server To Client Handshake{} ---", color_format::COLOR_GOLD, COLOR_WHITE);
                                                    println!("JWT: {:?}", jwt);
                                                    let jwt_split: Vec<&str> = jwt.split('.').collect();
                                        ...
                                        let compression_type = decrypted_stream.get_byte();
                                        println!("Compression Type: {}", if compression_type == 0 { format!("{}ZLIB{}", color_format::COLOR_LIGHT_PURPLE, COLOR_WHITE) } else if compression_type == 1 { format!("{}SNAPPY{}", color_format::COLOR_LIGHT_PURPLE, COLOR_WHITE) } else { format!("{}NONE{}", color_format::COLOR_LIGHT_PURPLE, COLOR_WHITE) });
                                        let decompressed = game_packet::decompress(&decrypted_stream.get_remaining().unwrap());
                                        ...
                                        while !decompressed_stream.feof() {
                                            ...
                                            println!("Packet ID: {}{}", color_format::COLOR_GOLD, packet_id);
                                            match packet_type {
                                                BedrockPacketType::ResourcePacksInfo => {
                                                    let resource_packs_info = resource_packs_info::decode(packet_stream.get_remaining().unwrap());
                                                    let mut rp_uuids = Vec::new();
                                                    println!("--- {}Resource Packs Info{} ---", color_format::COLOR_GOLD, COLOR_WHITE);
                                                    println!("Must Accept: {}", resource_packs_info.must_accept);
                                                    println!("Has Addons: {}", resource_packs_info.has_addons);
                                                    println!("Has Scripts: {}", resource_packs_info.has_scripts);
                                                    println!("Force Server Packs: {}", resource_packs_info.force_server_packs);
                                                ...
                                                BedrockPacketType::ResourcePackStack => {
                                                    println!("--- {}Resource Pack Stack{} ---", color_format::COLOR_GOLD, COLOR_WHITE);
                                                    // DATA SENT AS FRAGMENT
                                                    /*let rp_client_response = resource_pack_client_response::new(resource_pack_client_response::COMPLETED, rp_uuids).encode();
                                                ...
                                                BedrockPacketType::CreativeContent => {
                                                    println!("--- {}Creative Content{} ---", color_format::COLOR_GOLD, COLOR_WHITE);
                                                },
                                                BedrockPacketType::BiomeDefinitionList => {
                                                    println!("--- {}Biome Definition List{} ---", color_format::COLOR_GOLD, COLOR_WHITE);
                                                },
                                                BedrockPacketType::LevelChunk => {
                                                    println!("--- {}Level Chunk{} ---", color_format::COLOR_GOLD, COLOR_WHITE);
                                                },
                                                BedrockPacketType::PlayStatus => {
                                                    let play_status = play_status::decode(packet_stream.get_remaining().unwrap());
                                                    println!("--- {}Play Status{} ---", color_format::COLOR_GOLD, COLOR_WHITE);
